...
export async function fetchUsersForApproval(): Promise<User[]> {
	...
	const response = await auth.fetchWithAuth("http://localhost:8080/api/user/approvals?" + new URLSearchParams({schoolId: schoolId}), {
		method: 'GET',
	});
...
export async function approveUser(userId: number, role: Role): Promise<void> {
	...
	const response = await auth.fetchWithAuth("http://localhost:8080/api/user/approve?" + new URLSearchParams({schoolId: schoolId}), {
		method: "POST",
		body: JSON.stringify({
			userId: userId,
			role: role
